  ...
  let fetchData = async () => {
    try {
      ...
      const user = await axios.get(
        `https://63e61d3483c0e85a868ce98f.mockapi.io/api/v1/students/${params.id}`
      );
      setUsers(user.data);
      setLoading(false);
    } catch (error) {
    ...
    <div class="container">
      <div className="row">
        <div className="col-md-6 offset-md-3 border rounded p-4 mt-2 shadow">
          ...
          <h3 className="text-center m-4">Attendance:{users.attendance}</h3>
          <h3 className="text-center m-4">Gender:{users.gender}</h3>
          <h3 className="text-center m-4">Phone:{users.phone}</h3>

          <Link className="btn btn-outline-danger mx-2" to="/portal/dashboard">
            Back
          </Link>
        </div>
      </div>
